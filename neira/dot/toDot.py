...
def pairsToDot(graph, orders, links=None):
    ...
    for edge in orders:
        ...
        edge = (
            nodeName(edge.first)
            + " -> "
            + nodeName(edge.second)
            + '[label="'
            + marginStr
            + "\n"
            + edge.date.strftime("%m/%d")
            + '", weight="'
            + str(100 - int(margin))
            + '"'
            + colorStr
            + ', tooltip="'
            + reduceWhitespace(edge.tooltip.replace('"', ""))
            + '", URL="'
            + edge.url
            + '", random="random"]'
        )
...
def nodeName(name):
    return "".join(filter(str.isalnum, str(name).replace("_", "zzz"))).replace(
        "zzz", "_"
    )
